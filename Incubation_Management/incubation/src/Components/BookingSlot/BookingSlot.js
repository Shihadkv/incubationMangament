            ...
            <div className="d-flex sectionA ms-lg-5 mx-auto">
                {slotA &&
                    slotA.map((item) => {
                        ...
                        return (
                            <div className={item.isBooked ? 'slot  bg-secondary' : 'slot  bg-info'} onClick={() => { return (item.isBooked ? " " : handShow(item.slot, item.section)) }}></div>
                        )
                    })
                }
            </div>
            <div className="row ms-5">
                <div className="col-md-2 col-sm-6  ms-5 section">
                    {slotB &&
                        slotB.map((item) => {
                            return (
                                <div className={item.isBooked ? 'subSlot col-4  bg-secondary' : 'subSlot col-4  bg-info'} onClick={() => { return (item.isBooked ? " " : handShow(item.slot, item.section)) }}></div>
                            )
                        })}
                </div>
            ...
            <Modal show={show} onHide={handleClose}>
                ...
                <Modal.Footer>
                    <Button variant="secondary" onClick={()=>{
                        {handleClose(); swal("SLOT IS BOOKED")}
                    }}>
